...
ggml_status llama_context::graph_compute(
    ...
    // SLIM-ARC: Dynamic MADV switching (opt-in via SLIM_ARC_DYNAMIC_MADV=1).
    // Default: MADV_RANDOM only (set in init_mappings), no switching.
    // Dynamic: prefill→WILLNEED, decode→MADV_RANDOM (only on phase change).
    // Note: In llama-bench (separate prefill/decode runs), dynamic switching
    // hurts because WILLNEED pages get discarded on switch to MADV_RANDOM.
    // May help in real inference (continuous prefill→decode transition).
    if (getenv("SLIM_ARC_DISABLE") == nullptr &&
        getenv("SLIM_ARC_DYNAMIC_MADV") != nullptr) {
        static int current_madv = -1;
        int target_madv = batched ? 0 : 1;
        if (current_madv != target_madv) {
            slim_arc::switch_madvise_all(target_madv);
            current_madv = target_madv;
        }
    }
