...
readonly BENCHMARK="${SLIM_ARC_PI_BENCHMARK:-/home/yituodabian/data/llama-a23-39c88493/build/bin/llama-bench}"
readonly MODEL="${SLIM_ARC_PI_MODEL:-/home/yituodabian/data/Qwen3-Next-80B-A3B-Instruct-Q4_K_M.gguf}"
readonly RESULT_ROOT="${SLIM_ARC_PI_RESULT_ROOT:-/home/yituodabian/data/slim-arc-a23-results/2026-08-14-pi5-noswap-screen-r1}"
...
readonly CASES="${SLIM_ARC_PI_CASES:-control,top4}"
...
record_case_metadata() {
    ...
    {
        printf 'name=%s\n' "${name}"
        printf 'cross_layer_transition=%s\n' "${transition}"
        printf 'cross_layer_transition_topk=%s\n' "${topk}"
        printf 'pp=%s\n' "${PP}"
        printf 'tg=%s\n' "${TG}"
        printf 'threads=4\n'
        printf 'swap=off\n'
...
run_case() {
    ...
    local start end rc
    local -a transition_env=()

    if [[ "${transition}" == "1" ]]; then
        transition_env=(
            SLIM_ARC_CROSS_LAYER_TRANSITION=1
            "SLIM_ARC_CROSS_LAYER_TRANSITION_TOPK=${topk}"
        )
    fi

    mkdir -p "${result}"
    record_case_metadata "${result}" "${name}" "${transition}" "${topk}"
    sync
...
run_requested_cases() {
    ...
    for name in "${requested[@]}"; do
        case "${name}" in
            control) run_case control 0 0 || return ;;
