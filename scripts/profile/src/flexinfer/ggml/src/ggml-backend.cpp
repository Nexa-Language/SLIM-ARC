...
#ifndef NDEBUG
        fprintf(stderr, "%s: warning: slow copy from %s to %s\n", __FILE__, ggml_backend_buffer_name(src->buffer), ggml_backend_buffer_name(dst->buffer));
#endif
...
struct ggml_backend_registry {
    ...
    void register_backend(ggml_backend_reg_t reg) {
        ...
        fprintf(stderr, "%s: registered backend %s (%zu devices)\n",
            __FILE__, ggml_backend_reg_name(reg), ggml_backend_reg_dev_count(reg));
#endif
        GGML_LOG_INFO("%s:%d registered backend %s (%zu devices)\n",
            __FILE__, __LINE__, ggml_backend_reg_name(reg), ggml_backend_reg_dev_count(reg));
...
#ifndef NDEBUG
        fprintf(stderr, "%s: registered device %s (%s)\n", __FILE__, ggml_backend_dev_name(device), ggml_backend_dev_description(device));
#endif
...
static const size_t TENSOR_ALIGNMENT = 4096; // required for mmap as gguf only guarantees 32-byte alignment
...
static int ggml_backend_sched_backend_from_buffer(ggml_backend_sched_t sched, const struct ggml_tensor * tensor, const struct ggml_tensor * op) {
    ...
    fprintf(stderr, "%s: warning: no backend supports op %s with a weight with buffer type %s used in tensor %s, the weight will need to be copied\n",
        __FILE__, ggml_op_desc(tensor), ggml_backend_buffer_name(buffer), tensor->name);
#endif
...
static void ggml_backend_sched_split_graph(ggml_backend_sched_t sched, struct ggml_cgraph * graph) {
    ...
    if (sched->ctx == NULL) {
        GGML_ABORT("%s:%d failed to initialize context\n", __FILE__, __LINE__);
    }
