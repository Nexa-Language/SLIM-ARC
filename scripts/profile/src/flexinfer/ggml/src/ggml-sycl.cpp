...
struct ggml_sycl_pool_leg : public ggml_sycl_pool {
    ...
    void * alloc(size_t size, size_t * actual_size) override {
        ...
        if (!ptr) {
            fprintf(stderr, "%s: can't malloc %lu Bytes memory on device", __FILE__, look_ahead_size);
            return nullptr;
        }
...
static void ggml_sycl_op_get_rows(ggml_backend_sycl_context & ctx, const ggml_tensor *src0,
    ...
    switch (src0->type) {
        ...
        default:
            // TODO: k-quants
            fprintf(stderr, "%s: unsupported type: %s\n", __FILE__, ggml_type_name(src0->type));
            GGML_ABORT("fatal error");
            break;
    }
}
...
ggml_backend_sycl_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft,
    ...
    SYCL_CHECK(CHECK_TRY_ERROR(dev_ptr = (void *)sycl::malloc_device(
                                    size, *stream)));
    if (!dev_ptr) {
        fprintf(stderr, "%s: can't malloc %lu Bytes memory on device", __FILE__, size);
        return nullptr;
    }
...
ggml_backend_sycl_split_buffer_init_tensor(ggml_backend_buffer_t buffer,
    ...
    for (int i = 0; i < ggml_sycl_info().device_count; ++i) {
        ...
        */
        ...
        if (!buf) {
            char err_buf[1024];
            snprintf(err_buf, 1023, "%s: can't malloc %lu Bytes memory on device", __FILE__, size);
...
static ggml_status ggml_backend_sycl_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (!ok) {
            fprintf(stderr, "%s: error: op not supported %s (%s)\n", __FILE__, node->name, ggml_op_name(node->op));
        }
        GGML_ASSERT(ok);
    }
