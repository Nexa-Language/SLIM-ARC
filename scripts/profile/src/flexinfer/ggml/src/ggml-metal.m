...
static struct ggml_backend_metal_context * ggml_metal_init(ggml_backend_dev_t dev) {
    GGML_LOG_INFO("%s:%d allocating\n", __FILE__, __LINE__);

#if TARGET_OS_OSX && !GGML_METAL_NDEBUG
    // Show all the Metal device instances in the system
    NSArray * devices = MTLCopyAllDevices();
    for (id<MTLDevice> device in devices) {
        GGML_LOG_INFO("%s:%d found device: %s\n", __FILE__, __LINE__, [[device name] UTF8String]);
    }
    ...
    GGML_LOG_INFO("%s:%d picking default device: %s\n", __FILE__, __LINE__, [[device name] UTF8String]);
    ...
    // - if that fails, return NULL
    {
        ...
        GGML_LOG_INFO("%s:%d bundle = %s\n", __FILE__, __LINE__, bundle ? [[bundle bundlePath] UTF8String] : "nil");
        GGML_LOG_INFO("%s:%d GGML_METAL_EMBED_LIBRARY = %d, try_metallib = %d\n", __FILE__, __LINE__, GGML_METAL_EMBED_LIBRARY ? 1 : 0, try_metallib ? 1 : 0);

        NSString * path_lib = [bundle pathForResource:@"default" ofType:@"metallib"];
        if (path_lib == nil) {
            GGML_LOG_INFO("%s:%d default.metallib not found in bundle resources\n", __FILE__, __LINE__);
        } else {
            GGML_LOG_INFO("%s:%d default.metallib candidate path: %s\n", __FILE__, __LINE__, [path_lib UTF8String]);
        }
        if (try_metallib && path_lib != nil) {
            // pre-compiled library found
            NSURL * libURL = [NSURL fileURLWithPath:path_lib];
            GGML_LOG_INFO("%s:%d loading '%s'\n", __FILE__, __LINE__, [path_lib UTF8String]);

            metal_library = [device newLibraryWithURL:libURL error:&error];
            if (error) {
                GGML_LOG_ERROR("%s:%d error: %s\n", __FILE__, __LINE__, [[error description] UTF8String]);
                return NULL;
            }
        } else {
#if GGML_METAL_EMBED_LIBRARY
            GGML_LOG_INFO("%s:%d using embedded metal library\n", __FILE__, __LINE__);

            extern const char ggml_metallib_start[];
            extern const char ggml_metallib_end[];

            const NSUInteger metallib_size = (NSUInteger)(ggml_metallib_end - ggml_metallib_start);
            GGML_LOG_INFO("%s:%d embedded metallib size = %lu bytes\n", __FILE__, __LINE__, (unsigned long) metallib_size);

            NSString * src = [[NSString alloc] initWithBytes:ggml_metallib_start length:metallib_size encoding:NSUTF8StringEncoding];
            if (src == nil) {
                GGML_LOG_ERROR("%s:%d failed to create NSString from embedded metallib bytes\n", __FILE__, __LINE__);
                return NULL;
            }
#else
            GGML_LOG_INFO("%s:%d default.metallib not found, loading from source\n", __FILE__, __LINE__);

            NSString * path_source;
            NSString * path_resource = [[NSProcessInfo processInfo].environment objectForKey:@"GGML_METAL_PATH_RESOURCES"];

            GGML_LOG_INFO("%s:%d GGML_METAL_PATH_RESOURCES = %s\n", __FILE__, __LINE__, path_resource ? [path_resource UTF8String] : "nil");
            ...
            if (path_source == nil) {
                GGML_LOG_WARN("%s:%d error: could not use bundle path to find ggml-metal.metal, falling back to trying cwd\n", __FILE__, __LINE__);
                path_source = @"ggml-metal.metal";
            }

            GGML_LOG_INFO("%s:%d loading '%s'\n", __FILE__, __LINE__, [path_source UTF8String]);
            ...
            @autoreleasepool {
                ...
                GGML_LOG_INFO("%s:%d newLibraryWithSource returned %s (error = %s)\n",
                              __FILE__, __LINE__,
                              metal_library ? "non-null" : "null",
    ...
    // https://developer.apple.com/metal/Metal-Feature-Set-Tables.pdf
    {
        for (int i = MTLGPUFamilyApple1 + 20; i >= MTLGPUFamilyApple1; --i) {
            if ([device supportsFamily:i]) {
                GGML_LOG_INFO("%s:%d GPU family: MTLGPUFamilyApple%d  (%d)\n", __FILE__, __LINE__, i - (int) MTLGPUFamilyApple1 + 1, i);
                break;
            }
        }
        ...
        for (int i = MTLGPUFamilyCommon1 + 5; i >= MTLGPUFamilyCommon1; --i) {
            if ([device supportsFamily:i]) {
                GGML_LOG_INFO("%s:%d GPU family: MTLGPUFamilyCommon%d (%d)\n", __FILE__, __LINE__, i - (int) MTLGPUFamilyCommon1 + 1, i);
                break;
            }
        }
        ...
        for (int i = MTLGPUFamilyMetal3_GGML + 5; i >= MTLGPUFamilyMetal3_GGML; --i) {
            if ([device supportsFamily:i]) {
                GGML_LOG_INFO("%s:%d GPU family: MTLGPUFamilyMetal%d  (%d)\n", __FILE__, __LINE__, i - (int) MTLGPUFamilyMetal3_GGML + 3, i);
                break;
            }
        }
    }

    GGML_LOG_INFO("%s:%d simdgroup reduction support   = %s\n", __FILE__, __LINE__, ctx_dev->support_simdgroup_reduction ? "true" : "false");
    GGML_LOG_INFO("%s:%d simdgroup matrix mul. support = %s\n", __FILE__, __LINE__, ctx_dev->support_simdgroup_mm ? "true" : "false");
    GGML_LOG_INFO("%s:%d hasUnifiedMemory              = %s\n", __FILE__, __LINE__, ctx_dev->mtl_device.hasUnifiedMemory ? "true" : "false");
    ...
    if (@available(macOS 10.12, iOS 16.0, *)) {
        GGML_LOG_INFO("%s:%d recommendedMaxWorkingSetSize  = %8.2f MB\n", __FILE__, __LINE__, device.recommendedMaxWorkingSetSize / 1e6);
    }
#endif
...
#define GGML_METAL_ADD_KERNEL(e, name, supported) \
        if (supported) { \
            ...
            if (error) { \
                GGML_LOG_ERROR("%s:%d error: load pipeline error: %s\n", __FILE__, __LINE__, [[error description] UTF8String]); \
                [metal_library release]; \
                return NULL; \
            } \
        } else { \
            GGML_LOG_WARN("%s:%d skipping %-40s (not supported)\n", __FILE__, __LINE__, "kernel_"#name); \
        }
...
static void ggml_metal_free(struct ggml_backend_metal_context * ctx) {
    GGML_LOG_INFO("%s:%d deallocating\n", __FILE__, __LINE__);
...
static void ggml_metal_encode_node(
    ...
    //GGML_LOG_INFO("%s: encoding node %3d, op = %8s\n", __FILE__, idx, ggml_op_name(node->op));
    ...
    //GGML_LOG_INFO("%s: op - %s\n", __FILE__, ggml_op_name(dst->op));
    //if (src0) {
    //    GGML_LOG_INFO("%s: src0 - %4s [%5lld, %5lld, %5lld], %d, %s\n", __FILE__, ggml_type_name(src0t), ne00, ne01, ne02,
    //            ggml_is_contiguous(src0), src0->name);
    //}
    //if (src1) {
    //    GGML_LOG_INFO("%s: src1 - %4s [%5lld, %5lld, %5lld], %d, %s\n", __FILE__, ggml_type_name(src1t), ne10, ne11, ne12,
    //            ggml_is_contiguous(src1), src1->name);
    //}
    //if (dst) {
    //    GGML_LOG_INFO("%s: dst  - %4s [%5lld, %5lld, %5lld], 1, %s\n",  __FILE__, ggml_type_name(dstt),  ne0,  ne1,  ne2,
    //            dst->name);
    //}
    ...
    switch (dst->op) {
        ...
        case GGML_OP_UNARY:
            switch (ggml_get_unary_op(node)) {
                ...
                default:
                {
                    GGML_LOG_WARN("%s: node %3d, op = %8s not implemented\n", __FILE__, idx, ggml_op_name(dst->op));
...
static enum ggml_status ggml_metal_graph_compute(
            ggml_backend_t   backend,
        struct ggml_cgraph * gf) {
    GGML_LOG_INFO("%s:%d %d nodes are computed in METAL\n", __FILE__, __LINE__, gf->n_nodes);
    struct ggml_backend_metal_context        * ctx     = backend->context;
    struct ggml_backend_metal_device_context * ctx_dev = backend->device->context;
    ...
    @autoreleasepool {
        ...
        if (should_capture) {
            ...
            if (!ctx->capture_started) {
                ...
                if (![[MTLCaptureManager sharedCaptureManager] startCaptureWithDescriptor:descriptor error:&error]) {
                    GGML_LOG_ERROR("%s:%d error: unable to start capture '%s'\n", __FILE__, __LINE__, [[error localizedDescription] UTF8String]);
                } else {
        ...
        for (int i = 0; i < n_cb; ++i) {
            ...
            MTLCommandBufferStatus status = [command_buffer status];
            if (status != MTLCommandBufferStatusCompleted) {
                GGML_LOG_INFO("%s:%d command buffer %d failed with status %lu\n", __FILE__, __LINE__, i, status);
                if (status == MTLCommandBufferStatusError) {
                    GGML_LOG_INFO("error: %s\n", [[command_buffer error].localizedDescription UTF8String]);
                }
            ...
            if (ctx->abort_callback && ctx->abort_callback(ctx->abort_callback_data)) {
                GGML_LOG_INFO("%s:%d command buffer %d aborted", __FILE__, __LINE__, i);
...
static ggml_backend_buffer_t ggml_backend_metal_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft, size_t size) {
    ...
    if (size_aligned > 0 && (ctx->all_data == NULL || ctx->buffers[0].metal == nil)) {
        GGML_LOG_ERROR("%s: error: failed to allocate buffer, size = %8.2f MiB\n", __FILE__, size_aligned / 1024.0 / 1024.0);
        free(ctx);
    ...
    if (size_aligned <= device.maxBufferLength) {
        ...
        for (size_t i = 0; i < size; i += size_step) {
            ...
            if (size_step_aligned > 0) {
                ...
                if (ctx->buffers[ctx->n_buffers].metal == nil) {
                    GGML_LOG_ERROR("%s: error: failed to allocate buffer, size = %8.2f MiB\n", __FILE__, size_step_aligned / 1024.0 / 1024.0);
                    return false;
                }
            }
