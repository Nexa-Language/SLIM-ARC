...
inline static void * ggml_aligned_malloc(size_t size) {
    ...
    if (result != 0) {
        ...
        GGML_LOG_ERROR("%s: %s (attempted to allocate %6.2f MB)\n", __FILE__, error_desc, size/(1024.0*1024.0));
...
struct ggml_context * ggml_init(struct ggml_init_params params) {
    ...
    if (is_first_call) {
        ...
        // initialize GELU, Quick GELU, SILU and EXP F32 tables
        {
            ...
            GGML_PRINT_DEBUG("%s: GELU, Quick GELU, SILU and EXP tables initialized in %f ms\n", __FILE__, (t_end - t_start)/1000.0f);
        }
...
static struct ggml_object * ggml_new_object(struct ggml_context * ctx, enum ggml_object_type type, size_t size) {
    ...
    if (cur_end + size_needed + GGML_OBJECT_SIZE > ctx->mem_size) {
        GGML_LOG_WARN("%s: not enough space in the context's memory pool (needed %zu, available %zu)\n",
                __FILE__, cur_end + size_needed + GGML_OBJECT_SIZE, ctx->mem_size);
        assert(false);
        return NULL;
    }
        ...
        if (ctx->scratch.data != NULL) {
            ...
            if (ctx->scratch.offs + data_size > ctx->scratch.size) {
                GGML_LOG_WARN("%s: not enough space in the scratch memory pool (needed %zu, available %zu)\n",
                        __FILE__, ctx->scratch.offs + data_size, ctx->scratch.size);
                assert(false);
                return NULL;
            }
...
#ifdef FLEXINFER
size_t ggml_get_layer_index(const struct ggml_tensor * tensor, int is_param) {
    size_t result = -1;
...
static size_t ggml_flexinfer_get_aligned_read_size(struct ggml_tensor * tensor) {
    const size_t read_alignment = GGUF_DEFAULT_ALIGNMENT;
    const size_t tensor_size = ggml_nbytes(tensor);
    const size_t read_size = GGML_PAD(tensor_size, read_alignment);
    const size_t alloc_alignment = ggml_backend_buffer_get_alignment(tensor->buffer);
    const size_t alloc_size = GGML_PAD(ggml_backend_buffer_get_alloc_size(tensor->buffer, tensor), alloc_alignment);

    // Linux O_DIRECT requires the file offset, destination address, and read
    // length to be block-aligned. FlexInfer models are converted with 4096-byte
    // GGUF tensor alignment, and the CPU allocator reserves padded tensor space.
    if (tensor->off % read_alignment != 0) {
        GGML_ABORT("[FlexInfer] tensor '%s' offset %" PRIu64 " is not %zu-byte aligned; "
                "recreate the GGUF with --alignment %zu",
                tensor->name, (uint64_t) tensor->off, read_alignment, read_alignment);
    }
    if ((uintptr_t) tensor->data % read_alignment != 0) {
        GGML_ABORT("[FlexInfer] tensor '%s' data pointer %p is not %zu-byte aligned",
                tensor->name, tensor->data, read_alignment);
    }
    if (alloc_size < read_size) {
        GGML_ABORT("[FlexInfer] tensor '%s' allocation is too small for aligned read: "
                "alloc=%zu, read=%zu, tensor=%zu",
                tensor->name, alloc_size, read_size, tensor_size);
    }

    return read_size;
}

static void ggml_flexinfer_read_tensor(struct ggml_tensor * tensor) {
    const size_t read_size = ggml_flexinfer_get_aligned_read_size(tensor);

    ssize_t ret = pread(*(int*)(tensor->extra), tensor->data, read_size, tensor->off);
    if (ret == -1) {
        GGML_LOG_ERROR("[FlexInfer] failed to read tensor '%s': %s (fd=%d, size=%zu)\n",
                tensor->name, strerror(errno), *(int *)(tensor->extra), read_size);
    } else if (ret != (ssize_t) read_size) {
        GGML_LOG_ERROR("[FlexInfer] short read for tensor '%s': got %zd, expected %zu, offset=%" PRIu64 "\n",
                tensor->name, ret, read_size, (uint64_t) tensor->off);
    }
}
...
void ggml_prefetch_tensor(struct ggml_tensor * tensor) {
    if (prefetch_no_mmap == 0) {
        ...
        if (tensor->data != NULL)
            return;
#ifdef DEBUG
        GGML_LOG_DEBUG("[FlexInfer] alloc %s %zu\n", tensor->name, ggml_nbytes(tensor));
#endif
        while (tensor->data == NULL) {
           ggml_tallocr_alloc(global_alloc[0], tensor);
        }
        ggml_flexinfer_read_tensor(tensor);
    }
}

void ggml_prefetch_tensor_per_tensor(struct ggml_tensor * tensor, size_t index, int thread_index) {
    GGML_ASSERT(thread_num > 0);
    GGML_ASSERT(thread_index >= 0 && (uint32_t) thread_index < thread_num);

    if (prefetch_no_mmap == 0) {
        assert(tensor != NULL && tensor->data != NULL);
        size_t size = ggml_nbytes(tensor);
        volatile uint8_t tmp_sum = 0;
        size_t start = thread_index * size / thread_num;
        size_t end = (thread_index + 1) * size / thread_num;
        for (size_t k = start; k < end; k += BLOCK_SIZE)
            tmp_sum += ((uint8_t *)tensor->data)[k];
    } else {
#ifdef DEBUG
        GGML_LOG_DEBUG("[FlexInfer] alloc %s %zu\n", tensor->name, ggml_nbytes(tensor));
#endif
        if (atomic_cas_prefetch(&thread_alloc_status[index], 0, 1)) {
            ggml_tallocr_alloc(global_alloc[0], tensor);
            atomic_store_prefetch(&thread_alloc_status[index], 2);
        } else {
            while (atomic_load_prefetch(&thread_alloc_status[index]) != 2) {usleep(1000);}
        }
        const size_t read_alignment = GGUF_DEFAULT_ALIGNMENT;
        const size_t read_size = ggml_flexinfer_get_aligned_read_size(tensor);
        const size_t n_blocks = read_size / read_alignment;
        const size_t start_block = (size_t) thread_index * n_blocks / thread_num;
        const size_t end_block = ((size_t) thread_index + 1) * n_blocks / thread_num;
        const size_t start = start_block * read_alignment;
        const size_t size = (end_block - start_block) * read_alignment;
        if (size > 0) {
            ssize_t ret = pread(*(int*)(tensor->extra), (char*)tensor->data + start, size, tensor->off + start);
            if (ret == -1) {
                GGML_LOG_ERROR("[FlexInfer] failed to read tensor '%s': %s (fd=%d, size=%zu)\n",
                        tensor->name, strerror(errno), *(int *)(tensor->extra), size);
            } else if (ret != (ssize_t) size) {
                GGML_LOG_ERROR("[FlexInfer] short read for tensor '%s': got %zd, expected %zu, offset=%" PRIu64 "\n",
                        tensor->name, ret, size, (uint64_t) (tensor->off + start));
            }
        }
        if (atomic_add_prefetch(&thread_prefetch_status[index], 1) == thread_num - 1) {
            atomic_store_prefetch(&thread_alloc_status[index], 0);
            atomic_store_prefetch(&thread_prefetch_status[index], 0);
        }
    }
}
...
static int ggml_get_n_tasks(struct ggml_tensor * node, int n_threads) {
    ...
    switch (node->op) {
        ...
        default:
            {
                fprintf(stderr, "%s: op not implemented: ", __FILE__);
...
#ifdef FLEXINFER
        if (node_n > 0 && need_free) {
            if (prefetch_no_mmap && cgraph->n_nodes > 900) {
                if (atomic_cas_prefetch(&free_status, node_n - 1, node_n)) {
                    struct ggml_tensor * free_node = cgraph->nodes[node_n-1];
                    for (int i = 0; i < GGML_MAX_SRC; i++) {
                        if (free_node->src[i] != NULL) {
                            if (free_node->src[i]->need_prefetch == 1) {
#ifdef DEBUG
                                GGML_LOG_DEBUG("[FlexInfer] free tensor=%s, data=%p, size=%zu\n",
                                        free_node->src[i]->name,
                                        free_node->src[i]->data,
                                        ggml_nbytes(free_node->src[i]));
#endif
                                ggml_tallocr_free(global_alloc[0], free_node->src[i]);
                            }
                        } else
                            break;
                    }
...
#ifdef FLEXINFER
        if (prefetch_no_mmap <= 1 && cgraph->n_nodes > 900 && need_free) {
            int layer = ggml_get_layer_index(node, 0);
#ifdef DEBUG
            GGML_LOG_DEBUG("[FlexInfer] forward tensor=%s, layer=%d, prefetch_status=%d, forward_status=%d, thread=%p\n",
                    node->name, layer, prefetch_status, forward_status, (void *) pthread_self());
#endif
            while (layer == atomic_load_prefetch(&prefetch_status)) {
#ifdef DEBUG
                GGML_LOG_DEBUG("[FlexInfer] forward wait tensor=%s, layer=%d, prefetch_status=%d, forward_status=%d, thread=%p\n",
                        node->name, layer, prefetch_status, forward_status, (void *) pthread_self());
#endif
                usleep(1000);
            }
...
#ifdef SYNC_READ
        if (prefetch_no_mmap == 2 && cgraph->n_nodes > 900 && need_free) {
            for (int i = 0; i < GGML_MAX_SRC; i++) {
                //GGML_ASSERT(atomic_load_explicit(&state->threadpool->n_threads_cur, memory_order_relaxed) == 1);
                if (node->src[i] != NULL) {
                    if (atomic_cas_prefetch(&node->src[i]->need_prefetch, 1, 0)) {
                        if (node->src[i]->data == NULL) {
                            ggml_tallocr_alloc(global_alloc[0], node->src[i]);
                            ggml_flexinfer_read_tensor(node->src[i]);
                        }
                        atomic_cas_prefetch(&node->src[i]->need_prefetch, 0, 1);
                    }
                } else
                    break;
            }
    ...
    // check the magic before making allocations
    {
        ...
        for (uint32_t i = 0; i < sizeof(magic); i++) {
            if (magic[i] != GGUF_MAGIC[i]) {
                fprintf(stderr, "%s: invalid magic characters '%c%c%c%c'\n", __FILE__, magic[0], magic[1], magic[2], magic[3]);
                fclose(file);
                return NULL;
            }
        }
        ...
        for (uint64_t i = 0; i < ctx->header.n_tensors; ++i) {
            ...
            if (ggml_blck_size(info->type) == 0 || ne % ggml_blck_size(info->type) != 0) {
                fprintf(stderr, "%s: tensor '%s' of type %d (%s) number of elements (%" PRId64 ") is not a multiple of block size (%" PRId64 ")\n",
                        __FILE__, info->name.data, (int) info->type, ggml_type_name(info->type), ne, ggml_blck_size(info->type));
                fclose(file);
                gguf_free(ctx);
                return NULL;
            }
