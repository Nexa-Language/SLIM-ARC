...
static std::shared_ptr<socket_t> get_socket(const std::string & endpoint) {
    ...
    GGML_PRINT_DEBUG("[%s] connected to %s, sockfd=%d\n", __FILE__, endpoint.c_str(), sock->fd);
...
bool rpc_server::alloc_buffer(const std::vector<uint8_t> & input, std::vector<uint8_t> & output) {
    ...
    if (buffer != nullptr) {
        ...
        GGML_PRINT_DEBUG("[%s] size: %" PRIu64 " -> remote_ptr: %" PRIx64 ", remote_size: %" PRIu64 "\n", __FILE__, size, remote_ptr, remote_size);
...
bool rpc_server::set_tensor(const std::vector<uint8_t> & input) {
    ...
    GGML_PRINT_DEBUG("[%s] buffer: %p, data: %p, offset: %" PRIu64 ", size: %zu\n", __FILE__, (void*)tensor->buffer, tensor->data, offset, size);
...
bool rpc_server::copy_tensor(const std::vector<uint8_t> & input, std::vector<uint8_t> & output) {
    ...
    GGML_PRINT_DEBUG("[%s] src->buffer: %p, dst->buffer: %p\n", __FILE__, (void*)src->buffer, (void*)dst->buffer);
