...
int64_t get_example_targets_batch(
    ...
    // printf("%s: example_id=%d n_batch=%d n_train_samples=%zu\n", __FILE__, example_id, n_batch, n_train_samples);
    ...
    if (accum_step == 0) {
        ...
        *sched = learning_schedule(
        ...
        printf("%s: iter=%6d sample=%zu/%zu sched=%f loss=%f",
            __FILE__, opt->iter, std::min(1+train->shuffle_next_sample, train->shuffle_sample_count), train->shuffle_sample_count,
