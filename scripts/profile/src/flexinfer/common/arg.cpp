...
gpt_params_context gpt_params_parser_init(gpt_params & params, llama_example ex, void(*print_usage)(int, char **)) {
...
#ifdef FLEXINFER
    add_opt(llama_arg(
        {"-tp", "--threads-prefetch", "--threads_prefetch"}, "N",
        format("number of threads to use to prefetch (default: %d)", params.cpuparams.n_threads_prefetch),
        [](gpt_params & params, int value) {
            params.cpuparams.n_threads_prefetch = value;
            if (params.cpuparams.n_threads_prefetch < 0) {
                params.cpuparams.n_threads_prefetch = std::thread::hardware_concurrency();
            }
        }
    ).set_env("LLAMA_ARG_THREADS_PREFETCH"));

    add_opt(llama_arg(
        {"-am", "--available-mem", "--available_mem"}, "N",
        format("available memory (default: %.1f)", (double)params.available_mem),
        [](gpt_params & params, const std::string & value) {
            params.available_mem = std::stof(value);
        }
    ));
#endif
    ...
    add_opt(llama_arg(
        {"--dummy-input"}, "N",
        "replace the prompt with N dummy tokens (for benchmarking)",
        [](gpt_params & params, int value) {
            params.n_dummy_tokens = value;
        }
    ));
    add_opt(llama_arg(
        {"--keep"}, "N",
    ...
    add_opt(llama_arg(
        {"--dump-gemm-shapes"},
        "print GEMM dimensions for every compute graph",
        [](gpt_params & params) {
            params.dump_gemm_shapes = true;
        }
    ));
    add_opt(llama_arg(
        {"-nkvo", "--no-kv-offload"},
