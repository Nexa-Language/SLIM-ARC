...
class ModelBase:
    ...
    def __init__(self, dir_model: Path, ftype: gguf.LlamaFileType, fname_out: Path, *, is_big_endian: bool = False,
        ...
        self.gguf_writer.add_custom_alignment(alignment)
    ...
    @staticmethod
    def _tensor_sort_key(name: str) -> tuple[int, int, str]:
        parts = name.split(".")
        if name == "rope_freqs" or name.endswith(".rotary_emb.inv_freq"):
            return (0, 0, name)
        if any(part in {"embed_tokens", "tok_embeddings", "word_embeddings", "wte"} for part in parts):
            return (0, 1, name)

        for part in parts:
            if part.isdecimal():
                return (1, int(part), name)

        if parts[-1] in {"norm", "ln_f"} or name.endswith((".norm.weight", ".ln_f.weight")):
            return (2, 0, name)
        if parts[0] in {"lm_head", "output"}:
            return (3, 0, name)
        return (4, 0, name)
    ...
    def prepare_tensors(self):
        ...
        all_tensors = list(chain(self.generate_extra_tensors(), self.get_tensors()))
        if self.sort_tensors:
            all_tensors.sort(key=lambda item: self._tensor_sort_key(item[0]))
    ...
    def set_gguf_parameters(self):
        ...
        if "layer_types" in self.hparams:
            if all(lt == "full_attention" for lt in self.hparams["layer_types"]):
                # all layers are full attention for this model family, disable swa
