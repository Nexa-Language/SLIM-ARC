...
-   `--dump-gemm-shapes`: Print the tensor dimensions of every GEMM node in each compute graph. Helpful when inspecting model architecture or diagnosing shape mismatches.
-   `--dummy-input N`: Ignore the textual prompt and feed `N` dummy tokens instead—handy for stressing the model with a fixed-length synthetic input.
