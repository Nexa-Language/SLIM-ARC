...
static int load_imatrix(const std::string & imatrix_file, std::string & imatrix_dataset, std::unordered_map<std::string, std::vector<float>> & imatrix_data) {
    ...
    for (int i = 0; i < n_entries; ++i) {
        ...
        if (in.fail()) {
            printf("%s: failed reading name for entry %d from %s\n", __FILE__, i+1, imatrix_file.c_str());
            exit(1);
        }
        name_as_vec[len] = 0;
        ...
        if (getenv("LLAMA_TRACE")) {
            printf("%s: loaded data (size = %6d, ncall = %6d) for '%s'\n", __FILE__, int(e.size()), ncall, name.c_str());
        }
    }
    ...
    if (in.peek() != EOF) {
        ...
        printf("%s: imatrix dataset='%s'\n", __FILE__, imatrix_dataset.c_str());
    }
    printf("%s: loaded %d importance matrix entries from %s computed on %d chunks\n", __FILE__, int(imatrix_data.size()), imatrix_file.c_str(), m_last_call);
    return m_last_call;
}
...
static int prepare_imatrix(const std::string & imatrix_file,
    ...
    if (!imatrix_data.empty()) {
        printf("%s: have %d importance matrix entries\n", __FILE__, int(imatrix_data.size()));
    }
    return m_last_call;
}
...
int main(int argc, char ** argv) {
    ...
    for (; arg_idx < argc && strncmp(argv[arg_idx], "--", 2) == 0; arg_idx++) {
        ...
        } else if (strcmp(argv[arg_idx], "--align") == 0) {
            ++arg_idx;
            if (arg_idx < argc-1) {
                try {
                    params.align = std::stoi(argv[arg_idx]);
                } catch (const std::exception & e) {
                    fprintf(stderr, "%s: invalid align '%s' (%s)\n", __FILE__, argv[arg_idx], e.what());
                    return 1;
                }
            } else {
                usage(argv[0]);
            }
            printf("main: align = %ld\n", params.align);
        } else {
            usage(argv[0]);
        }
    }
    ...
    fprintf(stderr, "%s: quantizing '%s' to '%s' as %s", __FILE__, fname_inp.c_str(), fname_out.c_str(), ftype_str.c_str());
    if (params.nthread > 0) {
    ...
    // report timing
    {
        ...
        printf("\n");
        printf("%s: quantize time = %8.2f ms\n", __FILE__, t_quantize_us/1000.0);
        printf("%s:    total time = %8.2f ms\n", __FILE__, (t_main_end_us - t_main_start_us)/1000.0);
    }
