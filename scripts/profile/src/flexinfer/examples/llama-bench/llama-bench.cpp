...
#ifdef FLEXINFER
    std::vector<int> n_threads_prefetch;
    std::vector<float> available_mem;
#endif
...
#ifdef FLEXINFER
    printf("  -tp, --threads-prefetch <n>               (default: %s)\n", join(cmd_params_defaults.n_threads_prefetch, ",").c_str());
    printf("  -am, --available-mem <n> (GB)             (default: %s)\n", join(cmd_params_defaults.available_mem, ",").c_str());
#endif
...
#ifdef FLEXINFER
        } else if (arg == "-tp" || arg == "--threads-prefetch" || arg == "--threads_prefetch") {
            if (++i >= argc) {
                invalid_param = true;
                break;
            }
            auto p = string_split<int>(argv[i], split_delim);
            params.n_threads_prefetch.insert(params.n_threads_prefetch.end(), p.begin(), p.end());
        } else if (arg == "-am" || arg == "--available-mem" || arg == "--available_mem") {
            if (++i >= argc) {
                invalid_param = true;
                break;
            }
            auto p = string_split<float>(argv[i], split_delim);
            params.available_mem.insert(params.available_mem.end(), p.begin(), p.end());
#endif
        } else {
            invalid_param = true;
            break;
...
#ifdef FLEXINFER
    if (params.n_threads_prefetch.empty()) { params.n_threads_prefetch = {-1}; }
    if (params.available_mem.empty())      { params.available_mem = {2.0f}; }
#endif
...
#ifdef FLEXINFER
        int n_threads_prefetch = -1;
        float available_mem;
        bool use_mmap = false; // use read for prefetch
#else
        bool use_mmap;
#endif
    bool embeddings;
...
#ifdef FLEXINFER
        mparams.thread_num = n_threads_prefetch;
        mparams.available_mem = available_mem;
        mparams.ctx_size = n_prompt + n_gen;
#endif
        return mparams;
    }
...
#ifdef FLEXINFER
               n_threads_prefetch == other.n_threads_prefetch &&
               available_mem == other.available_mem &&
#endif
...
#ifdef FLEXINFER
    for (const auto & ntp : params.n_threads_prefetch)
    for (const auto & am : params.available_mem)
#endif
...
#ifdef FLEXINFER
        n_threads_prefetch = inst.n_threads_prefetch;
        available_mem = inst.available_mem;
#endif
...
#ifdef FLEXINFER
            "n_threads_prefetch", "available_mem",
#endif
            "embeddings",
...
#ifdef FLEXINFER
            std::to_string(n_threads_prefetch),
            std::to_string(available_mem),
#endif
...
#ifdef FLEXINFER
        if (field == "n_threads_prefetch") {
            return 3;
        }
        if (field == "available_mem") {
            return 5;
        }
#endif
        if (field == "test") {
...
#ifdef FLEXINFER
        if (params.n_threads_prefetch.size() > 1 || params.n_threads_prefetch != cmd_params_defaults.n_threads_prefetch) {
            fields.emplace_back("n_threads_prefetch");
        }
        if (params.available_mem.size() > 1 || params.available_mem != cmd_params_defaults.available_mem) {
            fields.emplace_back("available_mem");
        }
#endif
...
#ifdef FLEXINFER
            } else if (field == "available_mem") {
                snprintf(buf, sizeof(buf), "%.2f", t.available_mem);
                value = buf;
#endif
            } else if (field == "test") {
