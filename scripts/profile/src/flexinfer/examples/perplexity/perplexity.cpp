...
static results_perplexity perplexity_v2(llama_context * ctx, const gpt_params & params) {
    ...
    for (int i = 0; i < n_chunk; ++i) {
        ...
        if (i == 0) {
            ...
            LOG_INF("%s: %.2f seconds per pass - ETA ", __FILE__, t_total);
        ...
        //LOG_DBG("%s: using tokens %d...%d\n",__FILE__,params.n_ctx - params.ppl_stride + start, params.n_ctx + start);
...
static results_perplexity perplexity(llama_context * ctx, const gpt_params & params, const int32_t n_ctx) {
    ...
    LOG_INF("%s: tokenization took %g ms\n",__FILE__,1e-3*std::chrono::duration_cast<std::chrono::microseconds>(tim2-tim1).count());
...
static void hellaswag_score(llama_context * ctx, const gpt_params & params) {
    ...
    LOG_INF("%s : selecting %zu %s tasks.\n", __FILE__, hs_task_count, (randomize_tasks?"randomized":"the first")  );
...
static void multiple_choice_score(llama_context * ctx, const gpt_params & params) {
    ...
    if (params.multiple_choice_tasks == 0 || params.multiple_choice_tasks >= (size_t)n_task) {
        // Use all tasks
        tasks.resize(n_task);
        LOG_INF("%s: reading tasks", __FILE__);
    ...
    LOG_INF("%s: preparing task data", __FILE__);
    if (n_task > 500) {
        LOG("...");
        std::atomic<int> counter(0);
...
static void kl_divergence(llama_context * ctx, const gpt_params & params) {
    ...
    if (n_ctx > llama_n_ctx(ctx)) {
        LOG_ERR("%s: %s has been computed with %u, while the current context is %d. Increase it with -c and retry\n",
                __FILE__, params.logits_file.c_str(), n_ctx, params.n_ctx);
    }
    ...
    if (n_vocab != llama_n_vocab(llama_get_model(ctx))) {
        LOG_ERR("%s: inconsistent vocabulary (%d vs %d)\n", __FILE__, n_vocab, llama_n_vocab(llama_get_model(ctx)));
    }
...
int main(int argc, char ** argv) {
    ...
    if (params.n_ctx > n_ctx_train) {
        LOG_WRN("%s: model was trained on only %d context tokens (%d specified)\n",
                __FILE__, n_ctx_train, params.n_ctx);
    }
