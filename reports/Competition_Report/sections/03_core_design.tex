...
关键发现：prefill 阶段权重 I/O 占 85\%（需要全量加载），MADV\_RANDOM 阻止预读导致 -57\%；但 decode 阶段权重 I/O 仅占 2\%（MoE 稀疏），MADV\_RANDOM 让 98\% 未激活专家不进内存，带来 +850\%。由于交互式推理 90\%+ 的时间在 decode，tradeoff 高度有利。数据详见 Sec~\ref{sec:eval_core}（表~\ref{tab:core_iq4xs}）。
