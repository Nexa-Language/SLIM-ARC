...
\subsection{设计思路与整体架构}

\subsubsection{思路与创新}

端侧 MoE 大模型推理面临的核心挑战在于：模型权重远超物理内存，而 MoE 的稀疏激活特性使得每个 token 实际只需访问约 2\% 的专家权重。现有工作要么在用户态重新实现复杂的内存管理（如 FlexInfer 的 Direct I/O + 手动 buffer 管理），要么需要修改模型结构（如 PowerInfer 的 ReLU 稀疏化），要么假设专家为独立张量（如 MobileMoE），均未充分利用操作系统已有的虚拟内存机制。

SLIM-ARC 的新颖性主要在于：我们发现 \texttt{posix\_madvise(MADV\_RANDOM)} 这一标准 POSIX 接口能够让内核的 demand paging 与 MoE Router 的稀疏激活率天然匹配——关闭内核顺序预读后，仅被 Router 激活的 10/512 个专家权重通过 page fault 按需加载，其余 98\% 的未激活专家完全不占用物理内存，从而将 45\,GB 模型的 RSS 降至约 2\,GB。这一方案无需模型修改、无需用户态内存管理、无需专用硬件，仅依赖操作系统原生的虚拟内存机制即可实现。在此基础之上，我们进一步集成了量化压缩和计算融合两类优化，形成了 A（内核协同）$\to$ B（量化压缩）$\to$ C（计算融合）的级联协同：A 类释放物理内存，为 B 类的量化权重提供更多 cache 空间，B 类的更小模型又加速了 C 类的 attention 计算。

\subsubsection{整体架构}

SLIM-ARC 采用分层架构设计，如图\ref{fig:architecture}所示，自底向上分为三层。这一架构的核心思路是利用操作系统虚拟内存机制作为 MoE 稀疏性的天然接口，避免在用户态重新实现复杂的内存管理，而是通过 \texttt{madvise} 等标准 POSIX 接口~\cite{posix_madvise}控制内核的页面调度行为。
...
接下来，我们按照图\ref{fig:overview}中的三类优化来分别介绍各组件的设计：4.2 节介绍内核协同类优化（MADV\_RANDOM 按需加载与 REPACK 禁用），4.3 节介绍量化压缩类优化（IQ4\_XS 权重量化与 KV Cache 量化），4.4 节介绍计算融合类优化（FlashAttention 与 StreamingLLM KV Eviction）。

\subsection{内核协同类优化}

\subsubsection{mmap + MADV\_RANDOM 按需加载机制}
...
\begin{figure}[H]
    \centering
        \includegraphics[width=0.95\textwidth]{figures/madv_random.png}
...
\paragraph{prefill/decode 的 tradeoff}

MADV\_RANDOM 在 decode 阶段带来显著加速，但对 prefill 阶段有害。prefill 需要顺序读取所有层的全部权重，MADV\_RANDOM 阻止了内核的顺序预读，导致每个页面都要同步缺页。实测中 prefill 速度下降约 67\%，但 decode 速度提升 437--850\%。
...
\paragraph{prefill/decode 的 I/O 占比量化}

为理解 MADV\_RANDOM tradeoff 的量级，表\ref{tab:prefill_decode_io}量化了 80B 模型在 prefill 和 decode 两阶段的 I/O 与计算占比。

\begin{table}[H]
    \centering
    \caption{80B IQ4\_XS 各阶段 I/O 与计算占比（32\,GB, 8 threads, KV q4\_0, fa auto）}
    \label{tab:prefill_decode_io}
    \begin{tabular}{lccc}
        \toprule
        \textbf{指标} & \textbf{Prefill (pp64)} & \textbf{Decode (tg48)} & \textbf{说明} \\
        \midrule
        吞吐量 (t/s) & 12.99 & 5.16 & prefill 更高（批量并行） \\
        每 token 耗时 (ms) & 77 & 194 & decode 更慢（逐 token） \\
        权重 I/O 占比 & $\sim$85\% & $\sim$2\% & prefill 全量加载，decode 仅激活专家 \\
        KV I/O 占比 & $\sim$5\% & $\sim$40\% & decode 的 KV 随上下文增长 \\
        计算占比 & $\sim$10\% & $\sim$58\% & decode memory-bound，计算相对少 \\
        \bottomrule
    \end{tabular}
\end{table}

关键发现：prefill 阶段权重 I/O 占 85\%（需要全量加载），MADV\_RANDOM 阻止预读导致 -67\%；但 decode 阶段权重 I/O 仅占 2\%（MoE 稀疏），MADV\_RANDOM 让 98\% 未激活专家不进内存，带来 +850\%。由于交互式推理 90\%+ 的时间在 decode，tradeoff 高度有利。数据详见 Sec~\ref{sec:eval_core}（表~\ref{tab:core_iq4xs}）。

\subsubsection{禁用 GGML\_CPU\_REPACK}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \STATE $window \gets \text{window\_prefill if batched else window\_decode}$
...
\paragraph{MoE Router Hook 与跨层专家预测}
...
\begin{figure}[H]
    \centering
        \includegraphics[width=0.95\textwidth]{figures/moe_router_hook.png}
...
\subsubsection{FlashAttention 集成}

受综述~\cite{survey2026}Sec 4.1 “FlashAttention~\cite{flashattention2022} fuses attention IO-aware” 启发，SLIM-ARC 启用了 llama.cpp 内置的 FlashAttention 实现（\texttt{-fa auto} 参数）。FlashAttention 通过 IO-aware tiling 融合，将 attention 的 QK$^T$ 和 softmax(P)V 合并为单个 kernel，避免中间矩阵的显存读写，对 memory-bound 的 decode 阶段效果尤为显著。实测 80B decode 吞吐量提升 71.4\%（3.01 $\to$ 5.16 t/s），零代码改动、零精度损失。详细性能数据与 scaling 分析见 Sec~\ref{sec:eval_core}（表~\ref{tab:eval_flashattn} 和图~\ref{fig:fa_scaling}）。
...
\begin{figure}[H]
    \centering
        \includegraphics[width=0.95\textwidth]{figures/kv_eviction.png}
