...
\textbf{核心实验}展示完整 SLIM-ARC 在核心模型 80B 和三档环境下的性能；\textbf{附属实验}展示核心优化以外的其他数据；\textbf{消融实验}通过逐个关闭优化定位各组件贡献。所有实验串行执行（确保无内存竞争），冷启动（drop\_caches），数据可溯源至 \texttt{logs/ablation/full-rerun/}。
