...
\subsubsection{PowerInfer-2}

PowerInfer-2~\cite{powerinfer2_2024} 在智能手机上实现了大模型推理，利用异构计算（CPU + GPU）和稀疏访存模式优化。虽然其场景是 GPU 混合，但其"利用稀疏性减少 I/O"的核心思路与 SLIM-ARC 一致。

\subsubsection{端侧 LLM 综述}

综述~\cite{survey2024} 系统梳理了端侧 LLM 的压缩和系统优化技术，包括量化、剪枝、蒸馏、权重卸载、KV Cache 管理等方向。FlexInfer 的优化方向调研~\cite{flexinfer-optimize} 在此基础上提出了 5 个深度优化方向，SLIM-ARC 的设计直接参考了其中的 KV Cache 换页、MoE 预测预取、Tile 流水线等方向。
