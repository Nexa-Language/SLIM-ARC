...
\begin{table}[H]
  ...
  \begin{tabular}{p{0.16\textwidth}p{0.30\textwidth}p{0.46\textwidth}}
    ...
    8/17 & 完成 Ascend 910B4 最小兼容性探测 & 驱动与硬件枚举通过；CANN 用户态栈缺失且检测到其他任务占用，未执行算子、不报告 TPS \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{longtable}{p{0.28\textwidth}p{0.64\textwidth}}
  ...
  慢盘投机 I/O 大但吞吐无收益
    & A29 的 16\,MiB 预算将 issued/in-flight weight bytes 降低 98.44\% 且吞吐近似不变；A32/A33 进一步证明完全关闭 weight/expert speculative advice 后 wall 同为 505\,s。 \\
  热专家 cache 需要匹配 recurrent 访问
    & 512\,MiB LRU 将 evictions 降低 36.31\% 并保持吞吐；LFRU 增加 nonresident scan 56.33\%，两次命中准入使 prefill 下降约 54\%，均被拒绝。 \\
  \bottomrule
\end{longtable}
...
\begin{itemize}
  ...
  \item Ascend 最小探测：\path{docs/ascend_test_notes/2026-08-17/}；
