...
### 16GB cgroup（比赛高档环境，8 threads）

| 配置 | pp32 | tg8 | vs baseline |
|------|------|-----|------------|
| baseline (KV f16) | 1.04 | 0.18 | - |
| SLIM-ARC (KV f16) | 1.26 | 0.90 | **+400%** |
| **SLIM-ARC + KV q4_0** | **1.34** | **1.03** | **+472% (5.7×)** |

### 32GB 无限制（热缓存，8 threads）

| 配置 | pp32 | tg8 | 说明 |
|------|------|-----|------|
| SLIM-ARC (KV f16) | 1.90 | 1.24 | 接近流畅 |
| SLIM-ARC + KV q4_0 | - | - | 测试中 |

### 优化技术叠加效果

| 优化 | tg8 提升 | 机制 |
|------|---------|------|
| 禁用 repack | 基础（能跑） | 避免 OOM |
| + MADV_RANDOM | +400% | MoE 稀疏按需加载 |
| + KV q4_0 | +14% | KV 内存减半 |
| + 动态 MADV 切换 | 测试中 | prefill WILLNEED |

**结论**: 80B 在 16GB cgroup + KV q4_0 达到 **1.03 t/s decode**（baseline 的 5.7 倍）。32GB 热缓存达到 1.24 t/s 接近流畅。
