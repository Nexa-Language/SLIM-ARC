...
## 2026-08-21 专家复核材料索引完成

### 变更描述
- 读取最终项目报告、答辩 PPT 与项目全景总结，提取需要复查的系统机制和实验结论。
- 建立提交材料、原始实验记录、结构化数据与重点优化代码之间的统一索引，共校验
  103 个本地文件链接。
- 对 PPT 中的 2 GiB cgroup、64.5× 历史端点、消融 baseline、StreamingLLM 配置、
  KV 接线成熟度和 HiDevLab CPU-only 路径补充可核验边界。
- 文档采用面向专家的项目记录口吻，并提供关键源码符号和现场快速定位行号。

### 涉及文件
- `plan/40-v1-expert-review-material-index.md`
- `docs/expert-review/2026-08-22-expert-review-index.md`
- `ROADMAP.md`

### 决策原因
- 最终报告、答辩 PPT、设备实验记录和实现代码分布在不同目录，专家复核时需要统一入口。
- 复核材料应直接呈现数据口径、原始记录和实现位置，不依赖项目内部协作背景。
- 部分 PPT 页面压缩了不同实验批次或实现阶段，需要用原始日志和当前源码明确其适用范围。

---

## 2026-08-18 GitHub Pages 动态首屏与双视频入口

### 变更描述
- 将首页静态科研图降为背景材质，新增持续运行的页流、Router 脉冲、专家节点和
  鼠标引力场动画，使运行时机制成为首屏主体。
- 新增项目简介与决赛答辩两个 B 站内嵌播放器，并保留站外播放入口。
- 保留移动端与减少动态效果偏好下的降级路径。

### 涉及文件
- `site/index.html`
- `site/css/style.css`
- `site/js/hero-reactor.js`
- `site/js/main.js`
- `ROADMAP.md`

### 决策原因
- 原首屏动态粒子密度和对比度过低，视觉上仍像一张静态海报，无法直接表达
  SLIM-ARC 的实时调度特征。
- 两个视频承担不同叙事职责，需要在页面内明确区分并允许直接播放。

---

## 2026-08-18 GitHub Pages 科研视觉重构启动

### 变更描述
- 以“系统论文长卷”为首屏视觉母版，重构中文项目主页。
- 将运行时观测与跨设备证据地图整合为连续下滑叙事。
- 用真实同合同实验数据替换旧页面中的混合加速链和过时结论。
- 增加低密度、惯性衰减、可降级的随动粒子场。

### 涉及文件
- `plan/38-v1-github-pages-visual-rebuild.md`
- `site/`
- `ROADMAP.md`

### 决策原因
- 旧主页视觉层级、数据口径和决赛成果均已落后于最终报告。
- 新主页需要同时承担比赛展示、系统解释和跨设备证据入口三种职责。

---

## 2026-08-17 决赛报告最终视觉重构

### 变更描述
- 为摘要后的每个顶层章节增加独立分页，收束过短的设备小节标题。
- 以生成式科研示意图重绘系统架构、专家结算闭环和 RK3588 3 GiB 页访问机制；
  图内不放实验数字。
- 用可复算 Python 脚本生成 RK3588 主实验与动态策略、跨设备效应、消融总览和
  源码模块关系等多面板图。
- 为系统实现补充源码所有权图、runtime 生命周期图和预取—结算—回收伪代码。

### 涉及文件
- `plan/37-v1-finals-report-visual-refresh.md`
- `reports/Competition_Report_Finals/`
- `reports/archive/Competition_Report_Finals-2026-08-17-pre-visual-refresh/`
- `ROADMAP.md`

### 决策原因
- 旧稿的实现与实验章节图形密度不足，关键机制和跨设备结果需要更明确的视觉层次。
- 生成式图只适合表达机制；性能数字继续由确定性 Python 图和结构化证据承载。
- 历史版本需要可恢复，但不应进入官方 GitLab 发布白名单。

---

## 2026-08-17 决赛报告学术化重组启动

### 变更描述
- 冻结初赛报告目录，只重组决赛报告。
- 以系统论文的论证顺序重建引言、背景与相关工作、研究动机、系统设计、系统实现、
  实验评估、结论和附录。
- 将初赛至决赛的完整数据索引转化为附录表格；正文只保留与论点直接相关的数据。

### 涉及文件
- `plan/36-v1-finals-report-academic-restructure.md`
- `reports/Competition_Report_Finals/`
- `ROADMAP.md`

### 决策原因
- 当前决赛报告混合了设计、实现、实验和内部修订说明，读者无法沿问题到证据的逻辑
  理解系统贡献。
- 比赛报告需要用标准学术论文结构呈现全部技术工作，同时保留负面结果和跨设备边界。

---

## 2026-08-17 初赛至决赛实验数据统一整理启动

### 变更描述
- 盘点 WSL/x86、RK3588、Raspberry Pi 5、Mac/Colima 与华为 HiDevLab 的
  summary、JSON、原始日志和两版比赛报告。
- 新建统一数据索引，按实验合同和证据等级组织，不再要求读者从设备目录中自行
  拼接结论。
- 保留初赛历史高倍数结果，但把失效链接、跨缓存/跨环境数字和当前可引用结果明确
  分栏，避免继续混用。
...
### 决策原因
- 现有结果已经提交，但入口按日期和设备分散，队友无法快速确认 Mac、树莓派和
  华为实验是否存在。
- 比赛材料需要同时展示正结果、负结果和适用边界；统一索引比复制原始日志更适合
  评审和队内协作。
